...
void TurnToPoint::update(double dt) {
...
bool TurnToPoint::isFinished() const {
    return isFinishedStatus(status_);
}

MotionStatus TurnToPoint::getStatus() const {
    return status_;
}

void TurnToPoint::cancel() {
    if (isFinished()) {
        return;
    }
